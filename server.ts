...
async function startServer() {
  ...
  try {
    ...
    if (key && key !== "MY_GEMINI_API_KEY" && key.trim() !== "") {
      ...
      console.warn("⚠️ Warning: GEMINI_API_KEY is not defined or is placeholder. AI Waiter operates in smart offline simulation mode.");
    }
  } catch (err) {
    console.error("❌ Failed to initialize GoogleGenAI client:", err);
  }

  // API endpoint for restaurant assistant "Padavėjas Karolis"
...
Atsakinėk glaustai (iki 2-3 trumpų pastraipų), skaniai, linksmai ir lietuviškai. Jei vartotojas paklausia kita kalba, atsakyk ta kalba, bet išlaikyk tą patį linksmą mūsų šonkaulių mylėtojo charakterį. Formatuok atsakymus gražiai.`;
    ...
    if (!ai) {
      // Simulate clever answers if API key is missing
    ...
    try {
      // Convert messages format for Gemini 3.5 chat
      ...
      return res.json({ text: response.text || "Neturiu žodžių, koks skanus mūsų maistas!" });
    } catch (apiError: any) {
